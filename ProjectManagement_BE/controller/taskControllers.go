...
var AssignTask = func(w http.ResponseWriter, r *http.Request) {
	...
	wg.Add(len(request.ListUser))
	for i := range request.ListUser {
		go func(i int) {
			defer wg.Done()
			resp := m.AddMember2Task(UserID, *request.ListUser[i].UserID, *request.TaskID)
...
var UnassignTask = func(w http.ResponseWriter, r *http.Request) {
	...
	for i := range request.ListUser {
		resp := m.RemoveUserFromTask(UserID, *request.ListUser[i].UserID, *request.TaskID)
		temp = append(temp, resp)
	}
