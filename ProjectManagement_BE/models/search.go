...
func SearchProject(UserID uint, Query string, Status *uint, PageSize *uint, PageIndex *uint) (*[]Project, bool) {
	...
	if Status == nil {
		if PageSize != nil && PageIndex != nil {
			...
			err := GetDB().Table("projects").Joins("join user_projects on projects.id = user_projects.project_id").
				Where("user_projects.user_id = ? AND to_tsvector('english', projects.name) @@ plainto_tsquery('english', ?)", UserID, Query).
	...
	if PageSize != nil && PageIndex != nil {
		pageSize, offset := CalculatePaginate(*PageSize, *PageIndex)
		err := GetDB().Table("projects").Joins("join user_projects on projects.id = user_projects.project_id").
			Where("user_projects.user_id = ? AND projects.status = ? AND to_tsvector('english', projects.name) @@ plainto_tsquery('english', ?)", UserID, Status, Query).
			Offset(offset).Limit(pageSize).Find(project).Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				return nil, true
...
func SearchTask(UserID uint, Query string, Status *uint, PageSize *uint, PageIndex *uint) (*[]Task, bool) {
	...
	if Status == nil {
		if PageSize != nil && PageIndex != nil {
			...
			err := GetDB().Table("tasks").Joins("join user_tasks on tasks.id = user_tasks.task_id").
				Where("user_tasks.user_id = ? AND to_tsvector('english', tasks.name) @@ plainto_tsquery('english', ?)", UserID, Query).
	...
	if PageSize != nil && PageIndex != nil {
		pageSize, offset := CalculatePaginate(*PageSize, *PageIndex)
		err := GetDB().Table("tasks.").Joins("join user_tasks. on tasks..id = user_tasks.task_id").
			Where("user_tasks.user_id = ? AND tasks.status = ? AND to_tsvector('english', tasks..name) @@ plainto_tsquery('english', ?)", UserID, Status, Query).
			Offset(offset).Limit(pageSize).Preload("Subtasks").Find(task).Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				return nil, true
	...
	if Query != nil {
		if ProjectID != nil {
			if PageSize != nil && PageIndex != nil {
				...
				err := GetDB().Table("users").Distinct("id").
					Where("users.id not in (select user_id from user_projects where user_projects.project_id = ?) AND users.name LIKE ?", *ProjectID, "%"+*Query+"%").
			...
			if PageSize != nil && PageIndex != nil {
				...
				err := GetDB().Table("users").
					Where("users.name LIKE ? AND users.id <> ?", "%"+*Query+"%", UserID).
					Offset(offset).Limit(pageSize).Preload("Employee").Find(user).Error
				if err != nil {
					if err == gorm.ErrRecordNotFound {
						return nil, true
		...
		if ProjectID != nil {
			if PageSize != nil && PageIndex != nil {
				pageSize, offset := CalculatePaginate(*PageSize, *PageIndex)
				err := GetDB().Table("users").Distinct("id").
					Where("users.id not in (SELECT user_id from user_projects where user_projects.project_id = ?) AND users.id <> ?", *ProjectID, UserID).
					Offset(offset).Limit(pageSize).Preload("Employee").Find(user).Error
				if err != nil {
...
func SearchUserInProject(UserID uint, Query *string, ProjectID *uint, PageSize *uint, PageIndex *uint) (*[]User, bool) {
	...
	if Query != nil {
		if PageSize != nil && PageIndex != nil {
			pageSize, offset := CalculatePaginate(*PageSize, *PageIndex)
			err := GetDB().Table("users").Distinct("id").
				Where("users.id in (select user_id from user_projects where user_projects.project_id = ?) AND users.name LIKE ? AND users.id <> ?", *ProjectID, "%"+*Query+"%", UserID).
				Offset(offset).Limit(pageSize).Preload("Employee").Find(user).Error
			if err != nil {
				if err == gorm.ErrRecordNotFound {
					return nil, true
		...
		if PageSize != nil && PageIndex != nil {
			pageSize, offset := CalculatePaginate(*PageSize, *PageIndex)
			err := GetDB().Table("users").Distinct("id").
				Where("users.id in (select user_id from user_projects where user_projects.project_id = ?) AND users.id <> ?", *ProjectID, UserID).
				Offset(offset).Limit(pageSize).Preload("Employee").Find(user).Error
			if err != nil {
				if err == gorm.ErrRecordNotFound {
					return nil, true
...
func SearchUserTaskInProject(RequestUserID uint, UserID *uint, ProjectID *uint, Query *string, Status *uint, PageSize *uint, PageIndex *uint) (*[]Task, bool) {
	...
	if Status == nil {
		if PageSize != nil && PageIndex != nil {
			pageSize, offset := CalculatePaginate(*PageSize, *PageIndex)
			err := GetDB().Table("tasks").Joins("join user_tasks on tasks.id = user_tasks.task_id").
				Where("user_tasks.user_id = ? AND user_tasks.project_id = ? AND to_tsvector('english', tasks.name) @@ plainto_tsquery('english', ?)", UserID, *ProjectID).
				Offset(offset).Limit(pageSize).Preload("Subtasks").Find(task).Error
			if err != nil {
				if err == gorm.ErrRecordNotFound {
					return nil, true
	...
	if PageSize != nil && PageIndex != nil {
		pageSize, offset := CalculatePaginate(*PageSize, *PageIndex)
		err := GetDB().Table("tasks.").Joins("join user_tasks. on tasks..id = user_tasks.task_id").
			Where("user_tasks.user_id = ? AND tasks.status = ? AND user_tasks.project_id = ? AND to_tsvector('english', tasks.name) @@ plainto_tsquery('english', ?)", UserID, Status, *ProjectID, Query).
			Offset(offset).Limit(pageSize).Preload("Subtasks").Find(task).Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				return nil, true
...
func SearchUserInTask(UserID uint, Query *string, TaskID *uint, PageSize *uint, PageIndex *uint) (*[]User, bool) {
	...
	if Query != nil {
		pageSize, offset := CalculatePaginate(*PageSize, *PageIndex)
		err := GetDB().Table("users").Distinct("id").
			Where("users.id in (select user_id from user_tasks where user_tasks.task_id = ?) AND users.name LIKE ? AND users.id <> ?", *TaskID, "%"+*Query+"%", UserID).
			Offset(offset).Limit(pageSize).Preload("Employee").Find(user).Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				return nil, true
		...
		pageSize, offset := CalculatePaginate(*PageSize, *PageIndex)
		err := GetDB().Table("users").
			Where("users.id in (select user_id from user_tasks where user_tasks.task_id = ?) AND users.id <> ?", *TaskID, UserID).
			Offset(offset).Limit(pageSize).Preload("Employee").Find(user).Error
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				return nil, true
